                ...
                <div className="space-y-3 flex-1 overflow-y-auto pr-2 max-h-[300px] custom-scrollbar">
                    ...
                    {videoInfo.formats.slice(0, 5).map((format: any, idx: number) => (
                        <div key={idx} className="w-full">
                            <Button size="lg" asChild className="w-full h-14 rounded-xl font-bold px-6 shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all bg-white/5 hover:bg-white/10 border border-white/10 text-white group">
                                <a href={`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/download?url=${encodeURIComponent(format.url)}&title=${encodeURIComponent(videoInfo.title || 'video')}&ext=${encodeURIComponent(format.ext || 'mp4')}`} download className="flex items-center justify-between w-full">
                                    <div className="flex items-center">
                                        <Download className="h-5 w-5 mr-3 text-primary group-hover:scale-110 transition-transform" />
                                        <span className="text-lg">Download {format.resolution}</span>
                                    </div>
                                    <span className="text-xs font-bold opacity-40 uppercase tracking-widest">{format.ext}</span>
                                </a>
                            </Button>
                        </div>
                    ))}
